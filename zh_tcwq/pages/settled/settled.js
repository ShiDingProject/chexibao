...
Page({
    ...
    formSubmit: function(e) {
        ...
        console.log(t + " " + n);
        wx.getStorageSync("uniacid");
        var i = wx.getStorageSync("openid"), s = a.data.facilities, l = a.data.hotel_logo, c = a.data.wechat, u = a.data.yyzz, r = a.data.sfzzm, d = e.detail.value.business_name, p = e.detail.value.business_keyword, g = e.detail.value.address, m = a.data.time, f = a.data.time1, h = e.detail.value.business_number, y = e.detail.value.business_notice, x = e.detail.value.vr_demo;
        null == x && (x = "");
        var w = e.detail.value.textarea, v = a.data.coordinates, _ = a.data.store, T = a.data.store2, D = a.data.index, S = a.data.index_two, b = _[D], z = T[S];
        ...
        if (0 == uploaded.length) var A = ""; else A = uploaded.join(",");
        var M = "";
        if ("" != w && null != w || (w = ""), console.log(a.data, u, r), "" == d ? M = "请输入商家名称" : "" == p ? M = "请输入关键字" : "" == g ? M = "请输入详细地址" : "1" == a.data.xtxx.is_dnss && null == s ? M = "请勾选店内设施" : "1" == a.data.xtxx.is_dnss && 0 == s.length ? M = "请勾选店内设施" : "1" == a.data.xtxx.is_yysj && "00:00" == m ? M = "请输入营业开始时间" : "1" == a.data.xtxx.is_yysj && "00:00" == f ? M = "请输入营业结束时间" : "" == h ? M = "请输入联系电话" : "" == y ? M = "请输入公告说明" : "1" == a.data.xtxx.is_img && null == u ? M = "请上传营业执照照片" : "1" == a.data.xtxx.is_img && null == r ? M = "请上传法人身份证正面照片" : 540 <= w.length ? M = "内容超出" : 0 == a.data.sms ? null == a.data.num && (M = "请进行手机号验证") : 1 == a.data.sms && null == a.data.yz_code && (M = "请进行手机号验证"), 
        "" != M) wx.showModal({
            title: "提示",
            content: M,
            showCancel: !0,
            ...
            for (var F = 0, j = 0, U = 0, L = 0, B = 0, N = 0, O = 0; O < s.length; O++) if ("刷卡支付" == s[O]) F = 1; else if ("免费wifi" == s[O]) j = 1; else if ("免费停车" == s[O]) U = 1; else if ("禁止吸烟" == s[O]) L = 1; else if ("提供包间" == s[O]) B = 1; else if ("沙发休闲" == s[O]) N = 1;
            if (1 == I) {
                var R = 0 == a.data.countryIndex ? wx.getStorageSync("city") : "";
                console.log("city", R), P <= 0 ? app.util.request({
