...
Page({
    ...
    qrmd: function(t) {
      ...
      if (that.data.text.pay_type!=1){//餐饮 支付
        ...
        wx.showLoading({
          ...
          success: function (t) {
            console.log(a); console.log(e); console.log(s);
            console.log(t);
            var a = t.data;
            app.util.request({
        ...
        wx.showLoading({
          ...
          success: function (t) {
            console.log(a); console.log(e); console.log(o);
            console.log(t);
            if(t.data!=0){
              console.log(t);
              if (t.data == '积分不足') {
    ...
    onLoad: function(t) {
      ...
      var o = this;
      o.setData({ sceneStatus:1 });
      var a = '';//本地存储的url
      var e = '';//商家id
      var s = '';//用户id
      if (t.scene){
        // 扫码进入
        app.getUser(function(a){console.log('瞎写的a',a);});
        app.getUserInfo();
        e = t.scene;
        console.log('店铺id ');//店铺id 
        console.log(e);//店铺id
        app.util.request({
          url: "entry/wxapp/Url",
          cachetime: "0",
          success: function (w) {
            console.log(w), wx.setStorageSync("url", t.data), o.setData({
              url: w.data
            });
          }
        });
        app.util.request({
          ...
          success: function (q) {
            ...
            s = wx.getStorageSync("users").id;//用户id
            console.log(e);//店铺id
            console.log(s);//用户id
            console.log(a);//用户id
            o.setData({
              'text.store_id': e,
              'text.user_id': s,
              'text.pay_type': q.data.store[0].pay_type,
              'text.pay_integral': q.data.store[0].pay_integral
            });
            console.log(o.data.text);

            if (o.data.text.pay_type == "1") {
              o.setData({
                disabled: !1,
                total: Number(o.data.text.pay_integral).toFixed(0)
              });
            }
            console.log(s);
             o.lqyhq(s, e);
             app.util.request({
              ...
              success: function (t) {
                if (console.log("用户信息111", t.data), null != t.data.discount) var a = t.data.discount; else a = 100;
                o.setData({
                  userInfo: t.data,
                  discount: a,
        ...
        o.setData({
          'text.store_id': e,
          'text.user_id': s,
          'text.pay_type': t.pay_type,
          'text.pay_integral': t.pay_integral
        });
        console.log(o.data.text);
        ...
        if (o.data.text.pay_type == "1") {
          o.setData({
            disabled: !1,
            total: Number(t.pay_integral).toFixed(0)
          });
        }
